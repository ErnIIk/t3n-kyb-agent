...
- **1** — "complete the Quickstart" from the bounty scope, done against the live cluster.
- **2** — the delegation model is real: two separate principals, not one identity talking to itself.
- **3–5** — Walkthrough steps 2, 3 and the public-agent registration.
- **6–7** — the agent doing its job end to end, including the PII guarantee.
- **8** — the build verifies without any credentials, which is what makes it maintainable by someone
